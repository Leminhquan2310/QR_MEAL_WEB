                ...
                <div id="cart-container" class="${empty cart.items ? 'd-none' : ''}">
                    <c:forEach var="item" items="${cart.items}">
                        <div class="cart-item position-relative d-flex align-items-center justify-content-between p-2 border-bottom">
                            <!-- Nút xóa tinh tế -->
                            <button type="button" class="btn-remove" data-id="${item.product.id}" title="Xóa sản phẩm">
                            ...
                            <div class="text-danger fw-bold ms-3">
                                ...
                                <div class="input-group input-group-sm mt-1 quantity-group" style="max-width:120px;">
                                    <button class="btn btn-outline-secondary btn-decrease" data-id="${item.product.id}"
                                            type="button">-
                                    </button>
                                    <input type="text" class="form-control text-center quantity-input"
                                           value="${item.quantity}" data-id="${item.product.id}" readonly>
                                    <button class="btn btn-outline-secondary btn-increase" data-id="${item.product.id}"
                                            type="button">+
                                    </button>
                                </div>
                            </div>
